...
class UPSILoNT:
    def __init__(self, device=None, logger=None):
        ...
        self.f1_best_refit = 0.0
        self.mc_best_refit = 0.0
    ...
    def load(self, output_folder: str = None):
        ...
        self.n_final = pickle.load(
            open(os.path.join(output_folder, "n_final.pkl"), "rb")
        )
        self.min_values = pickle.load(
            open(os.path.join(output_folder, "min_params.pkl"), "rb")
        )
        self.norm_params = pickle.load(
            open(os.path.join(output_folder, "norm_params.pkl"), "rb")
        )
        self.label_encoder = pickle.load(
            open(os.path.join(output_folder, "label_encoder.pkl"), "rb")
        )
        ...
        if os.path.exists(os.path.join(output_folder, "state_dict.refit.pt")):
            genesis_model_path = os.path.join(output_folder, "state_dict.refit.pt")
        else:
    ...
    def predict(self, features: pd.DataFrame, return_prob: bool = False):
        ...
        # TODO: fine this code.
        features_in["period"], _ = apply_log10(
            features_in["period"], self.min_values["min_period"]
        )
        features_in["amplitude"], _ = apply_log10(
            features_in["amplitude"], self.min_values["min_amplitude"]
        )
        features_in["hl_amp_ratio"], _ = apply_log10(
            features_in["hl_amp_ratio"], self.min_values["min_hl_amp_ratio"]
        )
        features_in["kurtosis"], _ = apply_log10(
            features_in["kurtosis"], self.min_values["min_kurtosis"]
        )
        features_in["phase_cusum"], _ = apply_log10(
            features_in["phase_cusum"], self.min_values["min_phase_cusum"]
        )
        features_in["phase_eta"], _ = apply_log10(
            features_in["phase_eta"], self.min_values["min_phase_eta"]
        )
        features_in["quartile31"], _ = apply_log10(
            features_in["quartile31"], self.min_values["min_quartile31"]
        )
        features_in["skewness"], _ = apply_log10(
            features_in["skewness"], self.min_values["min_skewness"]
        )
        ...
        data_loader = torch.utils.data.DataLoader(
            data_set, batch_size=100, shuffle=False, num_workers=2, drop_last=False
        )
    ...
    def _train(
        ...
        features["hl_amp_ratio"], min_hl_amp_ratio = apply_log10(
            features["hl_amp_ratio"]
        )
        features["kurtosis"], min_kurtosis = apply_log10(features["kurtosis"])
        features["phase_cusum"], min_phase_cusum = apply_log10(features["phase_cusum"])
        features["phase_eta"], min_phase_eta = apply_log10(features["phase_eta"])
        features["quartile31"], min_quartile31 = apply_log10(features["quartile31"])
        features["skewness"], min_skewness = apply_log10(features["skewness"])
        features["slope_per90"], min_slope_per90 = apply_log10(features["slope_per90"])
        ...
        # Save.
        pickle.dump(
            self.n_final, open(os.path.join(output_folder, "n_final.pkl"), "wb")
        )
        ...
        pickle.dump(
            self.norm_params, open(os.path.join(output_folder, "norm_params.pkl"), "wb")
        )
        ...
        pickle.dump(
            self.label_encoder,
            open(os.path.join(output_folder, "label_encoder.pkl"), "wb"),
        )
        ...
        for i in range(n_iter):
            ...
            if verbose:
                self.logger.info(
                    "The overall best Mc and F1 using the "
                    "validation set: {0:.6f} and {1:.6f}".format(
                        self.mc_best, self.f1_best
                    )
                )
        ...
        fp = open(os.path.join(output_folder, "info.txt"), "w")
        fp.writelines(
            "# Best_Mc Median_Mc Mean_Mc Std_Mc " "Best_F1 Median_F1 Mean_F1 Std_F1\n"
        )
        fp.writelines(
    ...
    def _refit(
        ...
        for epoch in range(refit_n_epoch):
            ...
            if verbose:
                self.logger.info(
                    ("[refit: {0}] train Mc: {1:.6f}, " "learning rate {2:.1e}").format(
            ...
            pickle.dump(
                training_info,
                open(os.path.join(output_folder, "training_info.refit.pkl"), "wb"),
            )
            ...
            if training_mc > best_mc:
                ...
                # Save the model.
                torch.save(
                    net.state_dict(), os.path.join(output_folder, "state_dict.refit.pt")
                )
                self.net = net
                ...
                pickle.dump(
                    [true_label, predicted_label],
                    open(os.path.join(output_folder, "true_predicted.refit.pkl"), "wb"),
                )
        ...
        if verbose:
            self.logger.info(
                "[refit] The overall best Mc and F1 using the "
                "training set: {0:.6f} and {1:.6f}".format(best_mc, best_f1)
            )
    ...
    def _get_feature_importance(
        self, features: pd.DataFrame, labels: pd.DataFrame, device: str = "cuda:0"
    ):
        """Test purpose only."""
        ...
        x_train, x_test, y_train, y_test = train_test_split(
            features_norm, labels_encoded, test_size=0.2, stratify=labels_encoded
        )
